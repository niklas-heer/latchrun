...
Children receive `HOME`, `USER` and `LOGNAME` from the current OS account, `PATH=/usr/bin:/bin`, `LANG=C`, the profile's explicit non-secret `environment`, declared credentials, and an optional approved `ssh_auth_sock`. The account values come from the account database, never from the caller's environment; they are fixed by Latchrun, cannot be declared by a profile, and are absent when the account has no entry. `inspect` lists them as fixed variables. The home path does not grant additional filesystem access under the optional sandbox. `expose_environment` selects only declared non-secret values to show in inspection and the dashboard; it cannot expose credential values. Environment names use uppercase ASCII letters, digits and underscores. Loader, shell-control and internal control names are reserved, and secret/non-secret declarations cannot overlap. Do not put real secrets in profiles, arguments, names, paths or operation IDs.
...
- **Existing CLI logins:** tools you already signed in to, such as the [Google Workspace CLI](https://github.com/googleworkspace/cli) (`gws`) or [gogcli](https://github.com/openclaw/gogcli) (`gog`), keep using their own stored login. Without the sandbox they find their configuration through `HOME` and their Keychain items through `USER`, exactly as in your shell; `gws`, for example, names its Keychain entry after `USER` and would otherwise create a new key and lose its saved credentials. Approve the tool's native binary, not a script launcher: the npm `gws` launcher needs `node`, which the fixed `PATH` does not provide. Under the macOS sandbox, set `sandbox.keychain` and `network: "allow"`, and grant the tool's installation and configuration directories; see [Keychain access](docs/sandbox.md#keychain-access).
